...
bool FifoDataFile::Save(const std::string& filename)
{
  ...
  if (SConfig::GetInstance().bRAMOverrideEnable)
...
std::unique_ptr<FifoDataFile> FifoDataFile::Load(const std::string& filename, bool flagsOnly)
{
  ...
  if (flagsOnly)
  {
    ...
    SConfig::GetInstance().bRAMOverrideEnable = true;
    SConfig::GetInstance().bMEM1Size = header.mem1_size;
    SConfig::GetInstance().bMEM2Size = header.mem2_size;
