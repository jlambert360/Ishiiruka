...
id makeBackingLayer(id self, SEL _cmd)
{
  ...
  // This is an oddity, but alright. The SwapChain is already configured to be respective of Vsync,
  // but the underlying CAMetalLayer *also* needs to be instructed to respect it. This defaults to
  // YES; if we're not supposed to have vsync enabled, then we need to flip this.
  //
  // Notably, some M1 Macs have issues without this logic.
  //
  // I have absolutely no clue why this works, as MoltenVK also sets this property. Setting it
  // before giving the layer to MoltenVK seems to make it stick, though.
  if (!g_Config.IsVSync())
  {
    ...
    reinterpret_cast<void (*)(id, SEL, BOOL)>(objc_msgSend)(
        layer, sel_getUid("setDisplaySyncEnabled:"), NO);
  }

  // CAMetalLayer is triple-buffered by default; we can lower this to double buffering.
  //
  // (The only acceptable values are `2` or `3`). Typically it's only iMacs that can handle this, so
  // we'll just enable an ENV variable for it and document it on the wiki.
...
// purposes. To do this, it's not enough to just set `wantsLayer` to true - we need to also
// implement a few subclass methods, and tell the system we *want* the fast path.
//
// We have to inject a custom subclass as we can't modify the view (window_handle) in
// `PrepareWindow`, as that's a wxWidgets handle that relies on `drawRect:` being called for things
// to work. To work around this, we simply take the `window_handle` (i.e the view), create an
// instance of our `SLPMetalLayerView`, and attach that as a child view. `SLPMetalLayerView` should
// get the fast path, while everything else should stay golden.
Class getSLPMetalLayerViewClassType()
{
  ...
  if (SLPMetalLayerViewClass == nullptr)
  {
    ...
    // seem to hit a race condition with asynchronous queue submits. Rendering takes a slight hit
    // but this matters less in playback, and it's still better than OpenGL.
    ...
    setenv("MVK_CONFIG_PRESENT_WITH_COMMAND_BUFFER", "0", 0);
#endif
    SLPMetalLayerViewClass =
        objc_allocateClassPair((Class)objc_getClass("NSView"), kSLPMetalLayerViewClassName, 0);

    class_addMethod(SLPMetalLayerViewClass, sel_getUid("layerClass"), (IMP)getLayerClass, "@:@");
    class_addMethod(SLPMetalLayerViewClass, sel_getUid("wantsUpdateLayer"), (IMP)wantsUpdateLayer,
                    "v@:");
...
void VideoBackend::PrepareWindow(void* window_handle)
{
  ...
  id alloc = reinterpret_cast<id (*)(Class, SEL)>(objc_msgSend)(SLPMetalLayerViewClass,
                                                                sel_getUid("alloc"));

  auto rect = (CGRect){{0, 0}, {frame.size.width, frame.size.height}};
  id metal_view = reinterpret_cast<id (*)(id, SEL, CGRect)>(objc_msgSend)(
      alloc, sel_getUid("initWithFrame:"), rect);
  objc_msgSend(metal_view, sel_getUid("setWantsLayer:"), YES);

  // The below does: objc_msgSend(view, sel_getUid("setAutoresizingMask"), NSViewWidthSizable |
  // NSViewHeightSizable); All this is doing is telling the view/layer to resize when the parent
  // does.
